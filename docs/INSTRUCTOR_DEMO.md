...
the POSTURE platform stack**, which owns host port `5432` and the container name
`posturesec-db`. On that machine the lab's canonical database cannot start.
...
| 2 | Docker host-published port | `5432` | **`55432`** |
...
| 6 | backend `DB_PORT` | `5432` *(internal)* | **`55432`** *(published)* |
...
- **Host port bindings** are unique per interface: `55432` ≠ `5432`.
...
### Two values that must not be changed

- **DB host port `55432`** — the whole reason this branch exists.
- **Backend port `5000`** — `frontend/vite.config.js` hardcodes
  `target: 'http://localhost:5000'` and reads no environment variable. Moving
  the API would drag the frontend config with it, and students would no longer
  be able to reproduce what the instructor demonstrates.

`backend/.env` is git-ignored and cannot travel on a branch. The one line it
needs is `DB_PORT=55432`.
...
2. whether `lab-demo-db` is running and publishing `55432`;
